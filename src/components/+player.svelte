<script lang="ts">
  ...
  $: {
    ...
    if (videoUrl.indexOf('youtube.') !== -1) {
      ...
      parsedVideoUrl=`http://www.youtube.com/embed/${videoId}?enablejsapi=1&origin=${document.location.origin}`;
      isYouTube = true;

      // preload
      setTimeout(() => {
        (player as HTMLIFrameElement)?.contentWindow!.postMessage(JSON.stringify(
    ...
    countdownTimer = setInterval(() => {
      countdown = new Date(desiredTime.getTime() - Date.now() - videoTimestamp * 1000).toISOString().substring(11, 19);
    }, 1000);
  }
</script>
